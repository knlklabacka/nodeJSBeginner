...
// === Functions ==
// -- API --
const fetchAllWhispers = () => fetch('http://192.168.1.75:4000/api/v1/whisper', { headers: {Authentication: `Bearer ${accessToken}`}}).then((response) => response.json())
const deleteWhisper = (id) => fetch(`http://192.168.1.75:4000/api/v1/whisper/${id}`, { method: 'DELETE' })
const updateWhisper = (id, message) => fetch(`http://192.168.1.75:4000/api/v1/whisper/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) })
const createWhisper = (message) => fetch('http://192.168.1.75:4000/api/v1/whisper', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) })
